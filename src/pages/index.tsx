...
function filterOrdersByPeriodOffset(orders: Order[], period: string, offsetWeeks: number): Order[] {
  const now = new Date()
  now.setDate(now.getDate() - offsetWeeks * 7)
  return filterOrdersByPeriod(
    orders.map((o) => ({ ...o, date: shiftDateString(o.date, -offsetWeeks * 7) })),
    period
...
function computeWeeklySales(thisWeekOrders: Order[], lastWeekOrders: Order[]): WeeklySalesPoint[] {
...
function KpiCard({ label, value, trend, up }: KpiCardProps) {
  return (
    <Card className="bg-white rounded-2xl p-5 shadow-md hover:shadow-lg transition-all duration-300 hover:scale-105 border-0">
      ...
      <div className={`flex items-center gap-1 text-xs font-medium ${up === true ? "text-green-600" : up === false ? "text-red-500" : "text-gray-400"}`}>
      ...
      <div className="flex items-center gap-2 mt-3">
        ...
        <div className="flex gap-0.5">
          {[0, 0.2, 0.4, 0.6, 0.8, 1].map((t) => {
            ...
            const g = Math.round(242 - t * 105)
            const b = Math.round(228 - t * 174)
            return <div key={t} className="w-4 h-3 rounded-sm" style={{ backgroundColor: t === 0 ? "#F0FAF4" : `rgb(${r},${g},${b})` }} />
          })}
        </div>
...
export default function AdminDashboard() {
  ...
  const lastWeekOrders  = useMemo(() => filterOrdersByPeriodOffset(orders, selectedPeriod, 1), [orders, selectedPeriod])
  const weeklySalesData = useMemo(() => computeWeeklySales(filteredOrders, lastWeekOrders), [filteredOrders, lastWeekOrders])

  const weeklyChartData = useMemo(() =>
    weeklySalesData.map((p) => ({
      day: p.day,
      thisWeek: salesView === "sales" ? p.thisWeek : filteredOrders.filter((o) => DAYS_OF_WEEK[new Date(o.date).getDay()] === p.day).length,
      lastWeek: salesView === "sales" ? p.lastWeek : lastWeekOrders.filter((o) => DAYS_OF_WEEK[new Date(o.date).getDay()] === p.day).length,
      ...
      <main className="flex-1 p-8 pl-24">
        <div className="bg-[#FDFAF6] rounded-3xl p-8 min-h-[calc(100vh-5rem)]">
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="flex-1 max-w-2xl mx-8">
              <div className="relative">
                ...
                />
                <Button size="icon" className="absolute right-1 top-1 bg-gray-100 hover:bg-gray-200 rounded-full h-10 w-10 transition-all duration-300 hover:scale-105">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4 mb-8">
            ...
            <KpiCard label="Avg Order Value"  value={`₱${avgOrderValue.toFixed(2)}`}      trend="Per transaction" up={null} />
            <KpiCard label="Active Orders"    value={activeOrders.toLocaleString()}        trend="In progress" up={null} />
              ...
              <Card className="bg-white rounded-2xl p-6 shadow-md border-0">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex gap-2">
                    <Select value={salesView} onValueChange={(v) => setSalesView(v as "sales" | "orders")}>
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <LineChart data={weeklyChartData} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
                    ...
                    <Tooltip formatter={(value: number) => [yAxisFormatter(value)]} contentStyle={{ borderRadius: 12, border: "none", boxShadow: "0 4px 20px rgba(0,0,0,0.1)" }} />
                    <Line type="monotone" dataKey="thisWeek" stroke="#7C2D2D" strokeWidth={2.5} dot={{ r: 4, fill: "#7C2D2D" }} name="This period" />
                    <Line type="monotone" dataKey="lastWeek" stroke="#C8B8B8" strokeWidth={1.5} strokeDasharray="4 3" dot={{ r: 3, fill: "#C8B8B8" }} name="Previous period" />
                  </LineChart>
                </ResponsiveContainer>
              </Card>
            </div>
                ...
                {paymentBreakdown.length > 0 ? (
                  <>
                    <ResponsiveContainer width="100%" height={160}>
                      <PieChart>
                        <Pie data={paymentBreakdown} dataKey="count" nameKey="category" cx="50%" cy="50%" innerRadius={45} outerRadius={70} paddingAngle={3}>
                          {paymentBreakdown.map((entry, index) => (
                            <Cell key={entry.category} fill={PAYMENT_COLORS[entry.category] ?? ORDER_TYPE_COLORS[index % ORDER_TYPE_COLORS.length]} />
                          ))}
                        </Pie>
                        <Tooltip formatter={(value: number, name: string) => [value, name]} contentStyle={{ borderRadius: 12, border: "none", boxShadow: "0 4px 20px rgba(0,0,0,0.1)" }} />
                      </PieChart>
                    </ResponsiveContainer>
                    <div className="space-y-1.5 mt-2">
                      {paymentBreakdown.map((entry, index) => (
                        <div key={entry.category} className="flex items-center justify-between text-xs">
                          <div className="flex items-center gap-2">
                            <span className="w-2.5 h-2.5 rounded-sm inline-block" style={{ backgroundColor: PAYMENT_COLORS[entry.category] ?? ORDER_TYPE_COLORS[index % ORDER_TYPE_COLORS.length] }} />
              ...
              {orderTypes.length > 0 ? (
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={orderTypes} layout="vertical" margin={{ top: 0, right: 30, left: 10, bottom: 0 }}>
                    ...
                    <XAxis type="number" tick={{ fontSize: 11, fill: "#9B8E8E" }} axisLine={false} tickLine={false} tickFormatter={(v) => `${v}%`} domain={[0, 100]} />
                    <YAxis type="category" dataKey="type" tick={{ fontSize: 11, fill: "#9B8E8E" }} axisLine={false} tickLine={false} width={72} />
                    <Tooltip formatter={(value: number) => [`${value}%`, "Share"]} contentStyle={{ borderRadius: 12, border: "none", boxShadow: "0 4px 20px rgba(0,0,0,0.1)" }} />
