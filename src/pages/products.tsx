...
// ── Full menu tables — mirroring usersmenu.tsx exactly ───────────────────────
...
export default function Products() {
  ...
  // ── Fetch & merge — static is always the base so img/badge/etc. are never lost ──
  useEffect(() => {
    fetch('/api/products')
      .then(r => r.json())
      .then((d: any[]) => {
  ...
  return (
    <div style={{ fontFamily: "'Poppins', sans-serif", background: '#0e0c0a', minHeight: '100vh', color: '#f0ede8', position: 'relative' }}>
          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
            ...
            <div style={{ maxWidth: 1280, margin: '0 auto', padding: '12px 40px', display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <span style={{ fontSize: 12.5, color: 'rgba(240,237,232,0.55)', fontWeight: 400 }}>
                Fruit Soda available in <strong style={{ color: '#f5c842' }}>16oz (₱50)</strong> and <strong style={{ color: '#f5c842' }}>22oz (₱60)</strong>. Click <strong style={{ color: '#f0ede8' }}>Order</strong> to add to your order.
              </span>
            </div>
          </motion.div>
        )}
      ...
      <footer style={{ borderTop: '1px solid rgba(255,255,255,0.06)', padding: '52px 40px 40px', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ maxWidth: 1280, margin: '0 auto', paddingTop: 24, borderTop: '1px solid rgba(255,255,255,0.05)', textAlign: 'center' }}>
