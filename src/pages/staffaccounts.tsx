  ...
  const groupedAttendance = groupAttendance(onlineAttendance);
    ...
    useEffect(() => {
      ...
      const loadOnlineAttendance = async () => {
        try {
          const records = await authApi.getOnlineStaffAttendance(user.token);
          if (!isMounted) return;

          setOnlineAttendance(
            records
              .filter((record) => !/^(customer|user)$/i.test(record.role))
              .map((record) => ({
                ...
                {groupedAttendance.length === 0 ? (
                  <tr>
                    <td colSpan={9} style={{ textAlign: "center", padding: "64px 20px", fontSize: 13, color: "#cbd5e0" }}>
                      No online staff right now. Staff accounts appear automatically while logged in.
                    </td>
                  </tr>
                ) : (
