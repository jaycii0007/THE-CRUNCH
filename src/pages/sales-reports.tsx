...
import {
  ...
  X, RotateCcw, ChevronDown, Printer, TrendingUp,
} from "lucide-react";
...
type QuickKey = "today" | "yesterday" | "week" | "month" | "last7" | "last30" | "all";
...
function getQuickRange(key: QuickKey): { from: Date; to: Date } | null {
  ...
  if (key === "today") {
    ...
    f.setDate(1); f.setHours(0, 0, 0, 0); t.setHours(23, 59, 59, 999);
  } else if (key === "last7") {
    f.setDate(now.getDate() - 6); f.setHours(0, 0, 0, 0); t.setHours(23, 59, 59, 999);
  } else if (key === "last30") {
    f.setDate(now.getDate() - 29); f.setHours(0, 0, 0, 0); t.setHours(23, 59, 59, 999);
  }
  return { from: f, to: t };
}

function filterOrdersByRange(orders: Order[], from: Date | null, to: Date | null) {
  if (!from && !to) return orders;
  return orders.filter((o) => {
    const d = parseDateSafe(o.date);
    if (!d) return false;
    const day = new Date(d); day.setHours(0, 0, 0, 0);
    if (from) { const f = new Date(from); f.setHours(0, 0, 0, 0); if (day < f) return false; }
    if (to)   { const t = new Date(to);   t.setHours(0, 0, 0, 0); if (day > t) return false; }
    return true;
  });
}
...
function getRevenueForPeriod(logs: SaleLog[], period: Period) {
  const now = new Date();
...
function processRawRows(rows: RawOrderRow[]): { logs: SaleLog[]; orders: Order[] } {
  ...
  Object.entries(orderMap).forEach(([idStr, order]) => {
    ...
    const totalQty   = order.rows.reduce((s, r) => s + (Number(r.quantity) || 0), 0);
...
function triggerPrint(revenue: number, period: Period, logs: SaleLog[], orders: Order[]) {
  ...
  const rows = completedOrders.map((o, i) => `
    ...
    <body>
      ...
      <div class="revenue-hero">
        ...
        <div class="revenue-stats">
          <div class="stat-item">
            <p class="stat-number" style="color:#4ade80">${completed.length}</p>
            <p class="stat-label">Completed</p>
          </div>
          <div class="stat-item">
            <p class="stat-number" style="color:#fbbf24">${pending.length}</p>
            <p class="stat-label">Pending</p>
          </div>
          <div class="stat-item">
            <p class="stat-number" style="color:#f87171">${cancelled.length}</p>
            <p class="stat-label">Cancelled</p>
          </div>
          <div class="stat-item">
            <p class="stat-number" style="color:#60a5fa">${refunded.length}</p>
...
function DrumCol({ label, items, selectedIndex, onChange }: DrumColProps) {
  ...
  function applyY(y: number, animate: boolean) {
    ...
    innerRef.current.style.transition = animate ? "transform 0.42s cubic-bezier(0.15, 1, 0.3, 1)" : "none";
  ...
  function onMouseDown(e: React.MouseEvent) {
    ...
    s.dragging = true; s.startY = e.clientY; s.startCurY = s.curY;
    s.lastY = e.clientY; s.lastT = Date.now(); s.vel = 0;
  ...
  function onTouchStart(e: React.TouchEvent) {
    const s = state.current;
    s.dragging = true; s.startY = e.touches[0].clientY; s.startCurY = s.curY;
    s.lastY = e.touches[0].clientY; s.lastT = Date.now(); s.vel = 0;
    if (innerRef.current) innerRef.current.style.transition = "none";
  }
  ...
  function onTouchMove(e: React.TouchEvent) {
    ...
    s.lastY = e.touches[0].clientY; s.lastT = now;
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center" }}>
      <p style={{ fontSize: 10, fontWeight: 600, letterSpacing: 1, textTransform: "uppercase", color: "#94a3b8", marginBottom: 8, fontFamily: "'Poppins', sans-serif" }}>
        {label}
      </p>
      <div style={{ position: "relative", height: 180, width: "100%", overflow: "hidden" }}>
        <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 64, zIndex: 2, pointerEvents: "none", background: "linear-gradient(to bottom, #fff 20%, transparent)" }} />
        <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 64, zIndex: 2, pointerEvents: "none", background: "linear-gradient(to top, #fff 20%, transparent)" }} />
        <div style={{ position: "absolute", top: "50%", left: 6, right: 6, height: ITEM_H, marginTop: -ITEM_H / 2, borderRadius: 10, background: "rgba(74,28,28,0.07)", border: "0.5px solid rgba(74,28,28,0.18)", zIndex: 1, pointerEvents: "none" }} />
        <div
          ref={innerRef}
          ...
          onWheel={onWheel}
          style={{ display: "flex", flexDirection: "column", alignItems: "center", paddingTop: 72, paddingBottom: 72, transform: `translateY(${-selectedIndex * ITEM_H}px)`, cursor: "grab", userSelect: "none", willChange: "transform" }}
        >
          {items.map((item, i) => (
            <div
              key={i}
              style={{ height: ITEM_H, display: "flex", alignItems: "center", justifyContent: "center", width: "100%", fontSize: i === selectedIndex ? 17 : 14, fontWeight: i === selectedIndex ? 600 : 400, color: i === selectedIndex ? "#4A1C1C" : "#94a3b8", transition: "color 0.15s, font-size 0.15s", fontFamily: "'Poppins', sans-serif" }}
            >
              {item}
            </div>
          ))}
      ...
      {open && (
        <>
          <motion.div
            ...
            onClick={onClose}
            style={{ position: "fixed", inset: 0, zIndex: 200, background: "rgba(0,0,0,0.35)", backdropFilter: "blur(6px)" }}
          />
          <motion.div
            key="sheet"
            ...
            style={{ position: "fixed", top: "30%", left: "35%", transform: "translate(-50%, -50%)", zIndex: 201, width: 420, background: "#fff", borderRadius: 20, border: "0.5px solid #e2e8f0", paddingBottom: 32, fontFamily: "'Poppins', sans-serif", boxShadow: "0 10px 40px rgba(0,0,0,0.18)" }}
          >
            <div style={{ display: "flex", justifyContent: "center", paddingTop: 12, marginBottom: 4 }}>
              <div style={{ width: 36, height: 4, borderRadius: 99, background: "#e2e8f0" }} />
            </div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 20px 14px", borderBottom: "0.5px solid #f1f5f9" }}>
              <p style={{ fontSize: 15, fontWeight: 600, color: "#0f172a", margin: 0 }}>{title}</p>
              <button onClick={onClose} style={{ width: 28, height: 28, borderRadius: "50%", border: "0.5px solid #e2e8f0", background: "#f8fafc", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, color: "#94a3b8" }}>×</button>
            </div>
            <div style={{ display: "flex", gap: 0, padding: "8px 16px 0", justifyContent: "center", maxWidth: 360, margin: "0 auto" }}>
            ...
            <div style={{ display: "flex", gap: 10, padding: "20px 20px 0" }}>
              <button onClick={onClose} style={{ flex: 1, padding: "11px 0", borderRadius: 12, border: "0.5px solid #e2e8f0", background: "#f8fafc", fontSize: 13, fontWeight: 500, color: "#64748b", cursor: "pointer", fontFamily: "'Poppins', sans-serif" }}>Cancel</button>
              <button onClick={handleApply} style={{ flex: 2, padding: "11px 0", borderRadius: 12, border: "none", background: "#4A1C1C", fontSize: 13, fontWeight: 600, color: "#fff", cursor: "pointer", fontFamily: "'Poppins', sans-serif" }}>Apply</button>
            </div>
          </motion.div>
        </>
      )}
...
function RevenueDropdown({ period, setPeriod, logs, orders }: RevenueDropdownProps) {
    ...
    <>
      <AnimatePresence>
        {printToast && (
          <motion.div
            ...
            style={{ position: "fixed", top: 20, right: 24, zIndex: 9999, background: "#0f172a", borderRadius: 12, padding: "12px 18px", display: "flex", alignItems: "center", gap: 10, boxShadow: "0 8px 24px rgba(0,0,0,0.18)", fontFamily: "'Poppins', sans-serif" }}
          >
            <Printer size={14} color="#4ade80" />
            <span style={{ fontSize: 13, fontWeight: 500, color: "#fff" }}>Print dialog opened</span>
            <span style={{ fontSize: 11, color: "#64748b" }}>· {period} report</span>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div ref={ref} style={{ position: "relative", minWidth: 280, zIndex: 100 }}>
        <motion.div
          ...
          style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 16, padding: "18px 20px", cursor: "pointer", boxShadow: "0 2px 10px rgba(0,0,0,0.05)", userSelect: "none" }}
        >
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
            <span style={{ color: "#94a3b8", fontSize: 10, fontWeight: 600, letterSpacing: 1.2, textTransform: "uppercase" }}>Total Revenue</span>
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <span style={{ color: "#64748b", fontSize: 12, fontWeight: 500 }}>{period}</span>
              <motion.span animate={{ rotate: open ? 180 : 0 }} transition={{ duration: 0.2 }} style={{ color: "#94a3b8", fontSize: 10, display: "inline-block" }}>▼</motion.span>
            </div>
          </div>
          <AnimatePresence mode="wait">
            <motion.p
              ...
              style={{ color: "#0f172a", fontSize: 28, fontWeight: 700, margin: "0 0 8px", letterSpacing: -0.5 }}
            >
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {[
              ...
              <span key={s.label} style={{ fontSize: 10, color: s.color, fontWeight: 600, background: `${s.color}18`, padding: "2px 8px", borderRadius: 99 }}>
                {s.label}: {s.count}
              </span>
            ))}
          </div>
        ...
        <AnimatePresence>
          {open && (
            <motion.div
              ...
              style={{ position: "absolute", top: "calc(100% + 8px)", right: 0, background: "#fff", border: "1px solid #e2e8f0", borderRadius: 14, boxShadow: "0 8px 30px rgba(0,0,0,0.14)", zIndex: 9999, minWidth: "100%", overflow: "hidden" }}
            >
              <div style={{ padding: "10px 16px 6px", borderBottom: "1px solid #f1f5f9" }}>
                <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: 1.2, color: "#cbd5e1", textTransform: "uppercase" }}>Select Period</span>
              </div>
              ...
              {periods.map((p) => (
                <motion.div
                  ...
                  style={{ padding: "11px 16px", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: "1px solid #f8fafc" }}
                >
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <div style={{ width: 28, height: 28, borderRadius: 8, background: period === p ? "#0f172a" : "#f1f5f9", display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <TrendingUp size={12} color={period === p ? "#fff" : "#94a3b8"} />
                    </div>
                    <div>
                      <span style={{ color: period === p ? "#0f172a" : "#64748b", fontSize: 13, fontWeight: period === p ? 600 : 400, fontFamily: "'Poppins', sans-serif", display: "block" }}>{p}</span>
                      <span style={{ fontSize: 10, color: "#cbd5e1", fontFamily: "'Poppins', sans-serif" }}>₱{getRevenueForPeriod(logs, p).toLocaleString()} revenue</span>
                    </div>
                  </div>
              ...
              <motion.div
                ...
                onClick={handlePrint}
                style={{ padding: "12px 16px", cursor: "pointer", display: "flex", alignItems: "center", gap: 12, borderTop: "1px solid #f8fafc" }}
              >
                <div style={{ width: 36, height: 36, borderRadius: 10, background: "#4A1C1C", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, boxShadow: "0 2px 8px rgba(74,28,28,0.25)" }}>
                  <Printer size={16} color="#fff" />
                </div>
                <div style={{ flex: 1 }}>
                  <p style={{ margin: 0, fontSize: 13, fontWeight: 600, color: "#0f172a", fontFamily: "'Poppins', sans-serif" }}>Print Sales Report</p>
                  <p style={{ margin: 0, fontSize: 11, color: "#94a3b8", fontFamily: "'Poppins', sans-serif" }}>
                    {period} · ₱{revenue.toLocaleString()} · {completedCount} completed
                  </p>
                </div>
                <div style={{ fontSize: 10, fontWeight: 700, padding: "3px 8px", borderRadius: 6, background: "#f0fdf4", color: "#16a34a", border: "1px solid #bbf7d0", whiteSpace: "nowrap" }}>
                  PDF ready
                </div>
              </motion.div>

              <div style={{ padding: "8px 16px 12px", background: "#f8fafc" }}>
                <p style={{ margin: 0, fontSize: 10, color: "#94a3b8", lineHeight: 1.6, fontFamily: "'Poppins', sans-serif" }}>
                  Includes: revenue summary, completed orders, cashier breakdown, payment methods
                </p>
              </div>
            </motion.div>
          )}
...
function RefundModal({ open, log, order, onConfirm, onClose, loading }: RefundModalProps) {
  const id            = log?.id ?? (order ? `#${order.id}` : "");
  const product       = log?.product ?? (order ? order.items.map((i) => i.name).join(", ") : "");
      ...
      {open && (log || order) && (
        <>
          <motion.div
            ...
            style={{ position: "fixed", inset: 0, zIndex: 300, background: "rgba(0,0,0,0.35)", backdropFilter: "blur(4px)" }}
          />
          <motion.div
            key="modal"
          ...
          >
            ...
            <div style={{ padding: "24px 24px 22px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 18 }}>
                ...
                <div>
                  <p style={{ margin: 0, fontSize: 15, fontWeight: 700, color: "#0f172a" }}>Confirm Refund</p>
              ...
              <div style={{ background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 12, padding: "14px 16px", marginBottom: 20, display: "flex", flexDirection: "column", gap: 10 }}>
                {[
                  ...
                  <div key={f.label} style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16 }}>
                    <span style={{ fontSize: 11, color: "#94a3b8", fontWeight: 600, letterSpacing: 0.5, textTransform: "uppercase", flexShrink: 0, paddingTop: 2 }}>{f.label}</span>
                    <span style={{ fontSize: 13, fontWeight: 600, color: f.label === "Amount" ? "#dc2626" : "#1e293b", textAlign: "right", wordBreak: "break-word", maxWidth: "65%", lineHeight: 1.5 }}>{f.value}</span>
                  </div>
                ))}
              </div>

              <p style={{ margin: "0 0 20px", fontSize: 12, color: "#64748b", lineHeight: 1.65 }}>
                Are you sure you want to refund <strong style={{ color: "#0f172a" }}>₱{total.toLocaleString()}</strong> for <strong style={{ color: "#0f172a" }}>{product}</strong>? The order status will be updated to <span style={{ color: "#2563eb", fontWeight: 600 }}>Refunded</span>.
              </p>

              <div style={{ display: "flex", gap: 10 }}>
                <button onClick={onClose} disabled={loading} style={{ flex: 1, padding: "11px 0", borderRadius: 12, border: "1px solid #e2e8f0", background: "#f8fafc", fontSize: 13, fontWeight: 500, color: "#64748b", cursor: loading ? "not-allowed" : "pointer", fontFamily: "'Poppins', sans-serif", opacity: loading ? 0.6 : 1 }}>
                  Cancel
                </button>
                <button onClick={onConfirm} disabled={loading} style={{ flex: 2, padding: "11px 0", borderRadius: 12, border: "none", background: loading ? "#f87171" : "#dc2626", fontSize: 13, fontWeight: 600, color: "#fff", cursor: loading ? "not-allowed" : "pointer", fontFamily: "'Poppins', sans-serif", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, opacity: loading ? 0.85 : 1 }}>
                  {loading ? (
                    <><motion.span animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 0.8, ease: "linear" }} style={{ display: "inline-block" }}><RotateCcw size={13} /></motion.span>Processing…</>
                  ) : (
  ...
  return (
    ...
    >
      <motion.div
        ...
        style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 20px", cursor: "pointer", borderBottom: "1px solid #f1f5f9", backgroundColor: "#fff", transition: "background 0.15s" }}
      >
        <div style={{ width: 8, height: 8, borderRadius: "50%", background: TYPE_COLOR[log.type], flexShrink: 0 }} />
        ...
        <span style={{ color: TYPE_COLOR[log.type], fontSize: 11, fontWeight: 600, width: 70, textAlign: "center", flexShrink: 0 }}>{log.type}</span>
        ...
        <span style={{ color: STATUS_COLOR["Completed"], fontSize: 11, fontWeight: 600, width: 80, textAlign: "right", flexShrink: 0 }}>Completed</span>
        <motion.span animate={{ rotate: open ? 180 : 0 }} transition={{ duration: 0.2 }} style={{ color: "#cbd5e1", fontSize: 10, width: 16, textAlign: "center", flexShrink: 0 }}>▼</motion.span>
      </motion.div>
          ...
          >
            <div style={{ padding: "14px 20px 14px 42px", display: "flex", gap: 32, flexWrap: "wrap", alignItems: "flex-end" }}>
              {[
                ...
                <div key={f.label}>
                  <p style={{ color: "#94a3b8", fontSize: 10, fontWeight: 600, letterSpacing: 1, margin: "0 0 2px", textTransform: "uppercase" }}>{f.label}</p>
                  <p style={{ color: f.label === "Order Status" ? STATUS_COLOR["Completed"] : f.label === "Cashier" ? "#4A1C1C" : "#334155", fontSize: 13, fontWeight: f.label === "Cashier" ? 600 : 500, margin: 0 }}>{f.value}</p>
                </div>
              ))}
              {log.note && (
                <div>
...
function OrderRow({ order, index, onRefund }: { order: Order; index: number; onRefund: (order: Order) => void }) {
  const [open, setOpen] = useState(false);
  ...
  const otb = orderTypeStyle[order.orderType] ?? { bg: "#f1f5f9", text: "#64748b" };
  ...
  return (
    <>
      <TableRow className="border-gray-100 hover:bg-gray-50 transition-colors cursor-pointer" onClick={() => setOpen((v) => !v)}>
        ...
        <TableCell className="text-gray-800 font-semibold whitespace-nowrap">{fmtTime(order.date)}</TableCell>
        <TableCell>
        ...
        <TableCell className="text-blue-600 font-medium capitalize">{order.paymentCategory}</TableCell>
                ...
                <div style={{ background: "#f8fafc", borderBottom: "2px solid #e2e8f0", padding: "20px 24px 20px 36px" }}>
                  <div style={{ display: "flex", flexWrap: "wrap", marginBottom: 16 }}>
                    {[
                      ...
                      <div key={f.label} style={{ minWidth: 130, flex: "0 0 auto", paddingRight: 32, paddingBottom: 12, borderRight: i < arr.length - 1 ? "1px solid #e2e8f0" : "none", marginRight: i < arr.length - 1 ? 32 : 0 }}>
                        <p style={{ color: "#94a3b8", fontSize: 9, fontWeight: 700, letterSpacing: 1.2, margin: "0 0 4px", textTransform: "uppercase" }}>{f.label}</p>
                        <p style={{ margin: 0, fontSize: 13, fontWeight: ["Cashier","Subtotal"].includes(f.label) ? 700 : 500, color: (f as any).isStatus ? (statusStyle[order.status]?.text ?? "#64748b") : f.label === "Subtotal" ? "#0f172a" : f.label === "Cashier" ? "#4A1C1C" : "#334155" }}>
                          {f.value}
                        </p>
                      </div>
                    ))}
                  ...
                  <div style={{ marginBottom: canRefund ? 16 : 0 }}>
                    <p style={{ color: "#94a3b8", fontSize: 9, fontWeight: 700, letterSpacing: 1.2, margin: "0 0 8px", textTransform: "uppercase" }}>Items Ordered</p>
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                      {order.items.length > 0 ? order.items.map((item, idx) => (
                        <div key={idx} style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 8, padding: "6px 12px", fontSize: 12, fontWeight: 500, color: "#334155", display: "flex", alignItems: "center", gap: 6 }}>
                          <span style={{ background: "#f1f5f9", borderRadius: 4, padding: "1px 6px", fontSize: 11, fontWeight: 700, color: "#64748b" }}>×{item.quantity}</span>
                          {item.name}
                  ...
                  {canRefund && (
                    <motion.button
                      ...
                      style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "8px 18px", borderRadius: 10, border: "1px solid #fecaca", background: "#fef2f2", color: "#dc2626", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "'Poppins', sans-serif" }}
                    >
...
function OrdersTab({ orders, onRefund }: { orders: Order[]; onRefund: (order: Order) => void }) {
  const now = new Date();
  ...
  function clearRange() {
    setFromDate(null); setToDate(null); setActiveQuick("all"); setCurrentPage(1);
  }

  function applyQuick(key: QuickKey) {
    setActiveQuick(key);
    if (key === "all") { setFromDate(null); setToDate(null); }
    else { const range = getQuickRange(key)!; setFromDate(range.from); setToDate(range.to); }
    setCurrentPage(1);
  }
  ...
  const sorted       = [...filtered].sort((a, b) => (parseDateSafe(b.date)?.getTime() ?? -Infinity) - (parseDateSafe(a.date)?.getTime() ?? -Infinity));
          ...
          <div>
            ...
            {(hasRange || statusFilter !== "All") && (
              <p className="text-xs text-gray-400 mt-0.5">
                {filtered.length} order{filtered.length !== 1 ? "s" : ""} · <span className="text-green-600 font-medium">{completedCount} completed</span> · <span className="text-gray-600 font-medium">₱{totalRevenue.toLocaleString()} revenue</span>
              </p>
            )}
          </div>
        ...
        <div className="flex gap-2 flex-wrap mb-5">
          {ORDER_STATUS_FILTERS.map((s) => (
            <button
              ...
              className={`text-xs font-semibold px-3 py-1 rounded-full border transition-colors ${statusFilter === s ? statusActiveColor[s] : "bg-gray-50 text-gray-500 border-gray-200 hover:border-gray-300 hover:text-gray-700"}`}
            >
              {s}
            </button>
          ))}
        ...
        <Table>
          <TableHeader>
            <TableRow className="border-gray-200 hover:bg-transparent">
              {["Order ID", "Date", "Time", "Order Type", "Status", "Payment", "Amount"].map((h) => (
                <TableHead key={h} className={`text-gray-700 font-semibold${h === "Amount" ? " text-right" : ""}`}>{h}</TableHead>
              ))}
            </TableRow>
          </TableHeader>
          <TableBody>
...
export default function SalesReports() {
  ...
  const tabs: { key: TabKey; label: string; count: number; icon: React.ReactNode }[] = [
    {
      key: "logs", label: "Sales Logs", count: completedLogs.length,
      icon: (
    ...
    <div style={{ minHeight: "100vh", background: "#f8fafc", fontFamily: "'Poppins', sans-serif" }}>
      ...
      <div style={{ padding: "40px 40px 40px 88px" }}>
        ...
        >
          ...
          <RevenueDropdown period={period} setPeriod={setPeriod} logs={logs} orders={orders} />
        </motion.div>
        ...
        {/* Tabs */}
        <div style={{ display: "inline-flex", gap: 4, marginBottom: 20, background: "#f1f5f9", borderRadius: 99, padding: 4 }}>
          {tabs.map(({ key, label, count, icon }) => {
            ...
            return (
              <button
                ...
                style={{ display: "flex", alignItems: "center", gap: 7, padding: "7px 14px", borderRadius: 99, border: "none", background: active ? "#fff" : "transparent", cursor: "pointer", fontFamily: "'Poppins', sans-serif", fontSize: 13, fontWeight: active ? 600 : 500, color: active ? "#0f172a" : "#94a3b8", boxShadow: active ? "0 1px 6px rgba(0,0,0,0.10)" : "none", transition: "all 0.2s", whiteSpace: "nowrap" }}
              >
                <span style={{ color: active ? "#0f172a" : "#94a3b8", display: "flex", alignItems: "center" }}>{icon}</span>
                {label}
                <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", minWidth: 20, height: 20, padding: "0 6px", borderRadius: 99, fontSize: 11, fontWeight: 700, background: active ? "#0f172a" : "#cbd5e1", color: active ? "#fff" : "#64748b", transition: "all 0.2s" }}>
                  {count}
                </span>
              </button>
            );
        ...
        {activeTab === "logs" && (
          <>
            ...
            >
              {/* Table header */}
              <div style={{ display: "flex", alignItems: "center", gap: 14, padding: "10px 20px", borderBottom: "1px solid #f1f5f9", background: "#f8fafc" }}>
                ...
                {[
                  ...
                  <span
                    key={col.label}
                    style={{ color: "#94a3b8", fontSize: 10, fontWeight: 600, letterSpacing: 1, ...(col.flex ? { flex: col.flex } : { width: col.width, flexShrink: 0 }), ...(col.align ? { textAlign: col.align as any } : {}) }}
                  >
                    {col.label}
                  </span>
                ))}
              ...
              <AnimatePresence>
                {dates.length === 0 ? (
                  ...
                  dates.map((date) => {
                    ...
                    return (
                      <div key={date}>
                        <div style={{ padding: "8px 20px", background: "#f8fafc", borderBottom: "1px solid #f1f5f9", borderTop: "1px solid #f1f5f9", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                          <span style={{ color: "#64748b", fontSize: 11, fontWeight: 600, letterSpacing: 0.5 }}>{date}</span>
                          <span style={{ color: "#cbd5e1", fontSize: 11 }}>{entries.length} records · ₱{dayRevenue.toLocaleString()} revenue</span>
                        </div>
